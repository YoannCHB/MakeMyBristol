...
function validevocal(){
	if(vocalStat){
		...
		vocal.start(); //ON LANCE LA RECONNAISSANCE VOCALE
		vocalStat = true;
	}
}
	...
	for(var n = 0; n < word.length; n++){ //POUR LE NOMBRE DE MOTS DANS LA PHRASE ON REPETE EN BOUCLE
  	...
  	if(word[n] == "pourcent"){// Définition du mot 
		...
		if(word[n]+" "+word[n+1] == "hashtag rouge"){// Définition des mots
			phrase = phrase.replace('hashtag rouge', '<p style="color: Red; display: inline-block">');
		}
		
		if(word[n]+" "+word[n+1] == "hashtag jaune"){// Définition des mots 
			phrase = phrase.replace('hashtag jaune', '<p style="color: Yellow; display: inline-block">');
		}
		
		if(word[n]+" "+word[n+1] == "hashtag bleue"){// Définition des mots
			phrase = phrase.replace('hashtag bleue', '<p style="color: blue; display: inline-block">');
		}
		
		if(word[n] == "#bleue"){// Définition des mots
			phrase = phrase.replace('#bleue', '<p style="color: blue; display: inline-block">');
		}
		
			if(word[n]+" "+word[n+1] == "hashtag vert"){// Définition des mots
			phrase = phrase.replace('hashtag vert', '<p style="color: green; display: inline-block">');
		}
		
					if(word[n]+" "+word[n+1] == "hashtag vers"){// Définition des mots
			phrase = phrase.replace('hashtag vers', '<p style="color: green; display: inline-block">');
		}
		
					if(word[n]+" "+word[n+1] == "hashtag rose"){// Définition des mots
			phrase = phrase.replace('hashtag rose', '<p style="color: pink; display: inline-block">');
		}
		
			if(word[n]+" "+word[n+1]+" "+word[n+2] == "hashtag fluo jaune"){// Définition des mots
			phrase = phrase.replace('hashtag fluo jaune', '<p style="background: yellow; display: inline-block">');
		}
		
				if(word[n]+" "+word[n+1]+" "+word[n+2] == "hashtag fluo rouge"){// Définition des mots
			phrase = phrase.replace('hashtag fluo rouge', '<p style="background: red; display: inline-block">');
		}
		 
		if(word[n]+" "+word[n+1]+" "+word[n+2] == "hashtag fluo rose"){// Définition des mots
			phrase = phrase.replace('hashtag fluo rose', '<p style="background: pink; display: inline-block">');
		}
		
			if(word[n]+" "+word[n+1]+" "+word[n+2] == "hashtag fluo vert"){// Définition des mots
			phrase = phrase.replace('hashtag fluo vert', '<p style="background: green; display: inline-block">');
		}
		
		if(word[n]+" "+word[n+1]+" "+word[n+2] == "hashtag fluo bleu"){// Définition des mots
			phrase = phrase.replace('hashtag fluo bleu', '<p style="background: blue; display: inline-block">');
		}
		
			if(word[n]+" "+word[n+1]+" "+word[n+2] == "hashtag fluo violet"){// Définition des mots
			phrase = phrase.replace('hashtag fluo violet', '<p style="background: purple; display: inline-block">');
		}
		...
		if(word[n]+" "+word[n+1] == "hashtag arrêter"){// Définition des mots
			phrase = phrase.replace('hashtag arrêter', '</p>');
		}
		
		if(word[n]+" "+word[n+1] == "hashtag stop"){// Définition des mots
			phrase = phrase.replace('hashtag stop', '</p>');
		}
